function satcom_4axis_inverse_dynamics_build()
    % Açıklama: 4 Eksen SATCOM RNE Ters Dinamik Kurulum Scripti
    % =========================================================================
    ...
    cb1 = [ ...
        'try, ' ...
            ...
            'if f~=0, ' ...
                ...
                't = mat(:,2); t=t(:); ' ...
                'v_rpm = mat(:,4); v_rpm=v_rpm(:); ' ...
                'akim = mat(:,5); akim=akim(:); ' ...
                'akim = max(min(akim, 5), -5); ' ...
                'v = v_rpm * (pi/30); ' ...
                'tau_real = akim * 1.41; tau_real=tau_real(:); ' ...
                'v_filt = movmean(v, 50); ' ...
                'dt_array = diff(t); dt_array(dt_array==0)=1e-6; ' ...
                'a = zeros(size(v)); ' ...
                'a(1:end-1) = diff(v_filt) ./ dt_array; ' ...
                'a(end) = a(end-1); ' ...
                'a = movmean(a, 150); ' ...
                'z = zeros(size(t)); ' ...
                'dt_mean = mean(dt_array); ' ...
                'assignin(''base'',''in_qdot_az'',timeseries(v_filt,t)); ' ...
                ...
                'msgbox(''Veriler yüklendi! Akım pikleri sınırlandırıldı.'', ''BAŞARILI''); ' ...
            'end, ' ...
        'catch ME, ' ...
    ...
    cb2 = ['try, ', ...
               ...
               'tau_fark = tau_model - tau_real; ', ... % Aradaki farki (hatayi) hesapliyoruz
               ...
               'figure(''Name'', ''Dinamik Model vs Gerçek Test'', ''NumberTitle'', ''off'', ''Position'', [100, 100, 1000, 700]); ', ...
               ... % 1. GRAFİK: SADECE MODEL
               'subplot(2,2,1); ', ...
               'plot(zaman, tau_model, ''b'', ''LineWidth'', 1.5); ', ...
               'title(''1) Sadece Model Torku (Teorik)''); xlabel(''Zaman (s)''); ylabel(''Tork (Nm)''); grid on; ', ...
               ... % 2. GRAFİK: SADECE GERÇEK
               'subplot(2,2,2); ', ...
               'plot(zaman, tau_real, ''r'', ''LineWidth'', 1.5); ', ...
               'title(''2) Sadece Gerçek Sürücü Torku''); xlabel(''Zaman (s)''); ylabel(''Tork (Nm)''); grid on; ', ...
               ... % 3. GRAFİK: ÜST ÜSTE
               'subplot(2,2,3); ', ...
               'plot(zaman, tau_model, ''b'', ''LineWidth'', 1.5); hold on; ', ...
               'plot(zaman, tau_real, ''r--'', ''LineWidth'', 1.5); ', ...
               'title(''3) İkisi Birlikte Karşılaştırma''); xlabel(''Zaman (s)''); ylabel(''Tork (Nm)''); ', ...
               'legend(''Model'', ''Gerçek''); grid on; ', ...
               ... % 4. GRAFİK: FARK (HATA)
               'subplot(2,2,4); ', ...
               'plot(zaman, tau_fark, ''k'', ''LineWidth'', 1.2); ', ...
               'title(''4) Aralarındaki Fark (Model - Gerçek)''); xlabel(''Zaman (s)''); ylabel(''Hata (Nm)''); grid on; ', ...
               ...
               'SonucTablosu = table(zaman, tau_model, tau_real, tau_fark, ''VariableNames'', {''Zaman_saniye'', ''Model_Uretilen_Tork_Nm'', ''Gercek_Sistem_Tork_Referansi'', ''Fark_Hata_Nm''}); ', ...
               'writetable(SonucTablosu, ''Model_Tork_Sonuclari.csv''); ', ...
               'disp(''Detaylı grafikler çizildi ve Hata sütunu da CSV olarak kaydedildi!''); ', ...
           'catch ME, ', ...
    ...
    disp("SİSTEM HAZIR: RNE modeli CSV veri girişine uygun oluşturuldu.");
end
...
function rebuild_FramesAndAxes(path)
    ...
    % Paralel Eksen Teoremi için Phi Matrisleri:
    get_phi = @(r) [eye(3), zeros(3); [0 r(3) -r(2); -r(3) 0 r(1); r(2) -r(1) 0], eye(3)];
    
    r1 = [0; 0; 0];
    r2 = [0.180; 0; 0]; % AZ'den EL'ye uzanan 180mm kol
    r3 = [0.090; 0; 0]; % EL'den POL'e uzanan 90mm kol
    r4 = [0.060; 0; 0]; % POL'den CROSS'a uzanan 60mm kol
...
'function y = fcn(u)'
'% ========================================================================='
'% OUTBOARD (ILERI YONLU KINEMATIK) HESAPLAMA BLOGU'
'% Kaynak: Yesiloglu (2007) - Newton-Euler Ters Dinamik Algoritmasi'
'% Aciklama: Bu fonksiyonda, kaideden (base) uc is elemanina (end-effector) '
'% dogru uzaysal hiz (V) ve uzaysal ivme (Vdot) yayilimini hesapliyorum.'
'% ========================================================================='
'H = cell(4,1); Phi = cell(4,1); idx = 1;'
'% 1. Sinyal Ayiklama: Giris vektorunden H ve Phi matrislerini cekiyoruz'
'% H: Donme Eksen Vektorleri (Hareket Alt-uzayi matrisleri, 6x1)'
'for k=1:4, H{k} = u(idx:idx+5); idx = idx + 6; end'
'% Phi: Uzaysal Donusum (Transfer) Matrisleri (Paralel eksen tasimasi icin, 6x6)'
'for k=1:4, Phi{k} = reshape(u(idx:idx+35), 6, 6); idx = idx + 36; end'
'% qdot (Eklem Hizlari) ve qddot (Eklem Ivmeleri)'
'qdot = u(idx:idx+3); idx = idx + 4; qddot = u(idx:idx+3);'
'% 2. Baslangic Kosullari: Kaide (Base) sabit oldugu icin hiz ve ivme sifirdir'
'Vprev = zeros(6,1); Vdotprev = zeros(6,1); '
'V = cell(4,1); Vd = cell(4,1);'
'% 3. Ileriye Dogru Ozyineleme (Outboard Recursion: 1. eksenden 4. eksene)'
'for k=1:4'
'  Vk  = Phi{k}*Vprev + H{k}*qdot(k);'
'  Vdk = Phi{k}*Vdotprev + H{k}*qddot(k) + crm(Vk)*(H{k}*qdot(k));'
'  V{k}=Vk; Vd{k}=Vdk; Vprev=Vk; Vdotprev=Vdk;'
'end'
'% 4. Cikis Sinyallerini Birlestirme'
'y = zeros(48,1); idx = 1;'
'for k=1:4, y(idx:idx+5) = V{k}; idx = idx + 6; end  % Uzaysal Hizlar (V1..V4)'
'for k=1:4, y(idx:idx+5) = Vd{k}; idx = idx + 6; end % Uzaysal Ivmeler (Vdot1..Vdot4)'
'end'
'% ========================================================================='
...
'function X = crm(V)'
'    w = V(1:3); v = V(4:6); '
'    X = [skew(w), zeros(3,3); skew(v), skew(w)]; '
'end'
'function S = skew(a)'
...
'function y = fcn(u)'
'% ========================================================================='
'% INBOARD GERIYE DOGRU DINAMIK HESAPLAMA BLOGU'
'% Kaynak: Yesiloglu 2007 Newton-Euler Ters Dinamik Algoritmasi'
'% Aciklama: Bu fonksiyonda en uctaki is elemanindan kaideye dogru'
'% geriye isleyerek uzaysal kuvvetlerin yayilimini hesapliyorum.'
'% ========================================================================='
'Phi = cell(4,1); I_mat = cell(4,1); V = cell(4,1); Vd = cell(4,1); idx = 1;'
'% 1. Sinyal Ayiklama: Ileri kinematikten gelen ve sabit olan matrisleri aliyorum'
'% Phi uzaysal donusum matrisleridir ve 6x6 boyutundadir.'
'for k=1:4, Phi{k} = reshape(u(idx:idx+35), 6, 6); idx = idx + 36; end'
'% I_mat uzaysal eylemsizlik matrisleridir ve 6x6 boyutundadir.'
'for k=1:4, I_mat{k} = reshape(u(idx:idx+35), 6, 6); idx = idx + 36; end'
'% V ve Vd sirasiyla uzaysal hiz ve ivmelerdir.'
'for k=1:4, V{k} = u(idx:idx+5); idx = idx + 6; end'
'for k=1:4, Vd{k} = u(idx:idx+5); idx = idx + 6; end'
'% 2. Baslangic Kosullari: En uctaki uzva disaridan etki eden ekstra bir yuk yoktur.'
'F=cell(4,1); Fchild=zeros(6,1);'
'% 3. Geriye Dogru Ozyineleme: Islemler 4. eksenden 1. eksene dogru ilerler.'
'for k=4:-1:1'
'  % --- Newton-Euler Dinamik Denge Denklemi ---'
'  % Fk hesabi eylemsizlik kuvveti ile hiz capraz carpimlarinin toplanmasi '
'  % ve ardindan alt uzuvdan gelen yukun eklenmesiyle yapilir.'
'  Fk = I_mat{k}*Vd{k} + crf(V{k})*(I_mat{k}*V{k}) + Fchild;'
'  F{k}=Fk;'
'  '
'  % Hesaplanmis bu kuvveti donusum matrisinin transpozu ile ebeveyn uzva tasiyorum.'
'  if k>1, Fchild = Phi{k}'' * Fk; end'
'end'
'% 4. Cikis Sinyallerini Birlestirme: F1 den F4 e kadar uzaysal kuvvetler birlestirilir.'
'y = zeros(24,1); idx = 1;'
...
'% YARDIMCI FONKSIYONLAR'
'% ========================================================================='
'% Uzaysal Kuvvet Capraz Carpim Operatoru'
'% Hizin matris formunun negatif transpozudur ve kuvvetler icin kullanilir.'
'function X=crf(V), X=-crm(V)''; end'
'% Uzaysal Hiz Capraz Carpim Operatoru'
'function X=crm(V), w=V(1:3); v=V(4:6); X=[skew(w) zeros(3,3); skew(v) skew(w)]; end'
'% Antisimetrik Matris Donusumu'
